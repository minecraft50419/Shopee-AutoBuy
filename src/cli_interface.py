...
def CreatePlanPage():
    ...
    console.print('0=貨到付款  1=信用卡金融卡  2=銀行轉帳',
                  justify="center", style="bold bright_black")
    console.print("-----------------------------",
                  justify="center", style="bold")
    console.print("")
    ...
    for i in copy_data_template["payment_method"]:
        if i == "0":
            copy_data_template["payment_method"][a] = "貨到付款"
        if i == "1":
            copy_data_template["payment_method"][a] = "信用卡/金融卡"
        if i == "2":
            copy_data_template["payment_method"][a] = "銀行轉帳"
        a += 1
    ...
    console.print("由先到後排列", justify="center")
    console.print('輸入數字 並用 "空格" 區隔', justify="center")
    console.print(' 0=7-11 1=萊爾富 2=全家 3=OK超商 4=黑貓宅急便',
                  justify="center", style="bold bright_black")
    console.print("-----------------------------",
                  justify="center", style="bold")
    console.print("")
    ...
    for i in copy_data_template["delivery_method"]:
        if i == "0":
            copy_data_template["delivery_method"][a] = "7-11"
        if i == "1":
            copy_data_template["delivery_method"][a] = "萊爾富"
        if i == "2":
            copy_data_template["delivery_method"][a] = "全家"
        if i == "3":
            copy_data_template["delivery_method"][a] = "OK MART"
        if i == "4":
            copy_data_template["delivery_method"][a] = "黑貓宅急便"
        a += 1
    ...
    get_time = input("  搶購時間: ")
    copy_data_template["times"] = int(
        datetime.strptime(get_time, '%Y/%m/%d %H:%M').timestamp())
...
def All_Plan_Page():
    ...
    for data in listData:
        table.add_row(str(i), data['times'], data['website'], data['items_values'],
                      data['payment_method'][0], data['delivery_method'][0])
        i += 1
...
def MainMenu():
    ...
    console.print(Panel(Padding("will50419", (0, 5)),
                  title="目前帳號"), justify="center")

    console.print(" ")

    console.print(Panel(Padding("7.25蝦皮商城狂購節", (1, 5)),
                  title="2021/07/25 12:00"), justify="center")
    ...
    if num == '1':
        ...
        console.print("輸入錯誤", style="bold red")
